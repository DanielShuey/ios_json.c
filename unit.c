...
{
	jnode *n = parse(json);
	assert(!strcmp(n->get("string")->to.s, "Hello, World!"));
	...
	assert(!strcmp(
	    n->get("object")->get("nested_array")->to.a[4]->get("key")->to.s,
	    "value"));
	assert(!strcmp(
